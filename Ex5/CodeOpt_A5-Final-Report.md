...
The assembly code is generated by the following command:
...
The O3 optimization level, which is the highest one, perform serious optimizing operation. In the above assembly code, the loop has been unrolled, in addition to that, function inlining and automatic vectorization has been performed.
...
The loop structure are similar to the previous case in question a. However, the mathematic operation is different from the above. With -mfpmath=387, the assembly code produced look much more clean and seem to be more efficient.
